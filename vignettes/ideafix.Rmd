...
We can now extract some statistics on the output class labels.
...
As can be seen, RF classifies all the 148 variants as deaminations. The classification of XGBoost is almost the same, with the exception of two non-deaminations.

More interesting than directly relying the class labels, we can work with the rankings. This may not the best dataset for this example, but let us imagine we need to select five of these C:G $>$ T:A variants to see whether they are relevant in the pathogenesis of the cancer disease the specimen comes from. In such case, 
